...
class RandomWorkoutTab(QWidget):
    ...
    def generate_workout(self):
        ...
        selected_exercises = {body_part: random.choice(
            [exercise for exercise in data if exercise['body_part'] == body_part]) for body_part in body_parts}
        ...
        for exercise in selected_exercises.values():
            ...
            exercise_label = QLabel(
                f"{exercise['body_part']} - {exercise['exercise']}")
    ...
    def save_to_database(self):
        ...
        if user_id_query:
            ...
            for i in range(self.scroll_layout.count()):
                exercise_widget = self.scroll_layout.itemAt(i).widget()
                if exercise_widget:
                    body_part, exercise_name = exercise_widget.layout().itemAt(
                        0).widget().text().split(" - ")
                    set_details_layout = exercise_widget.layout().itemAt(1).layout()
                    ...
                    weights_reps = {f"set{j + 1}_weight": weights_reps.get(f"set{j + 1}_weight", None) for j in
                                    range(5)} | \
                                   {f"set{j + 1}_reps": weights_reps.get(
                                       f"set{j + 1}_reps", None) for j in range(5)}
